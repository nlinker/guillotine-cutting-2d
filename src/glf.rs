...
use crate::{
    ...
    model::{Piece, PieceSpec, Placement, PlacementSpec, ProblemSpec, SolutionSpec},
};
...
// == Partial solving (GLF pre-placement) =======================================
...
/// more room for whatever algorithm consumes the remaining pieces.
const ITERATION_BUDGET: usize = 10_000;

/// Result of GLF partial solving: a deterministic placement of the largest pieces
/// onto whole sheets, plus the reduced spec for everything GLF did not place.
pub struct GlfPreplaceResult {
    ...
    /// Reduced spec for the pieces that GLF did not place.
...
/// Deterministically place large pieces using GLF, one sheet per step.
///
...
fn find_max_fitting_prefix(pieces: &[Piece], flat_indices: &[usize], query_w: u32, max_h: u32) -> (usize, u32) {
    let mut best_k = 0;
    let mut best_h = 0;
...
mod preplace_tests {
    ...
    fn glf_preplace_skip_when_no_fit() {
        // Sheet 10x10, kerf=0. Single piece 5x5 fits in 10x10. Expect 1 sheet.
